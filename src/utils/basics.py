...
def _extraer_info_columna(nombre):
    """
    Extrae fecha y hora de un nombre de columna en cualquiera de los dos formatos:
      - Viejo: /2023-03-29_07-30.png
      - Nuevo: 2023-03-29_07-30
    Retorna (fecha, hora, minutos) o None si no matchea.
    """
    patron = r"(\d{4}-\d{2}-\d{2})_(\d{2})-(\d{2})"
...
def obtener_nombres_con_H_M(lista_nombres,  hora_exacta=None, rango_horas=None):
    ...
    for nombre in lista_nombres:
        info = _extraer_info_columna(nombre)
        if info is None:
            continue
        _, hora, minutos = info
