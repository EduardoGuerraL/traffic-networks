...
class NetworkData:
    ...
    def getMaxParticlesPerUrns(self, factor_escala = 1.876):
        ...
        largo_de_calles = long_of_streets(self.position_of_vertices, self.Edges)
    ...
    def plotNodes(self, dataframe, column_name:str, directed:bool = True ,name_to_save_file: str = None, width_node:float = 2):
        ...
        if directed is True:
            ...
            nx.draw_networkx_edges(self.DiGraphNodes, self.position_of_vertices, width= 0.3 , arrowsize=5, node_size=15)
            nodes=nx.draw_networkx_nodes(self.DiGraphNodes, self.position_of_vertices, node_size= width_node, node_color=indices_ordenado, cmap= plt.cm.Reds)
            plt.axis('off')
                
        #grafica para no direccionados
        elif directed is not True:
            nx.draw_networkx_edges(self.GraphNodes, self.position_of_vertices, width= 0.3, node_size=15)
            nodes=nx.draw_networkx_nodes(self.GraphNodes, self.position_of_vertices, node_size= width_node, node_color=indices_ordenado, cmap= plt.cm.Reds)
